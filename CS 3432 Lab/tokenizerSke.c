...
char *copy_str(char *inStr, short len){
   ...
   char * word = (char *)malloc(len*sizeof(char*));      //allocating memory space for my array
   for(int i = 0; i < len; i++) {
      word[i] += inStr[i];
      //printf("%d ",i); 
   }
...
char** tokenize(char* str){
   ...
   for(int i = 0; i < count_tokens(str)+1; i++){
...
void print_all_tokens(char** tokens){
   for(int i = 0; non_delim_character(*tokens[i]); i++) {
      printf("tokens[%d] = ""%s\n", i, tokens[i]);
   }
}
void main() {
